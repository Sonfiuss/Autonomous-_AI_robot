...
class Simulator {
    constructor() {
        ...
        // Real robot bridge
        this.realMode   = false;
        this.realPose   = null;   // { x_cm, y_cm, theta_deg }
        this.posePoller = null;

        this.setupEventListeners();
        this.setupRealRobotUI();
        this.render();
    ...
    // ── Real Robot UI ──────────────────────────────────────────────────────────
    setupRealRobotUI() {
        // Inject toggle button vào toolbar nếu chưa có
        if (!document.getElementById('realRobotBtn')) {
            const btn = document.createElement('button');
            btn.id = 'realRobotBtn';
            btn.textContent = '🤖 Real Robot: OFF';
            btn.style.cssText = 'margin-left:8px;padding:4px 10px;background:#444;color:#fff;border:1px solid #888;border-radius:4px;cursor:pointer;';
            btn.addEventListener('click', () => this.toggleRealMode(btn));
            const toolbar = document.querySelector('.toolbar') || document.querySelector('.controls') || document.body;
    ...
    toggleRealMode(btn) {
        ...
        if (this.realMode) {
            btn.textContent = '🤖 Real Robot: ON';
            btn.style.background = '#1a7a1a';
            this.startPosePolling();
            this.log('Real robot mode ON — clicks will move physical robot', 'success');
        } else {
            btn.textContent = '🤖 Real Robot: OFF';
            btn.style.background = '#444';
            this.stopPosePolling();
            this.realPose = null;
            this.log('Real robot mode OFF', 'info');
        }
    }

    startPosePolling() {
        this.posePoller = setInterval(async () => {
            try {
    ...
    stopPosePolling() {
        if (this.posePoller) { clearInterval(this.posePoller); this.posePoller = null; }
    }

    drawRealRobot() {
        if (!this.realPose) return;
        const pos = this.grid.worldToCanvas(this.realPose.x_cm, this.realPose.y_cm);
        const theta = this.realPose.theta_deg * Math.PI / 180;
        const ctx = this.ctx;
        ctx.save();
        ...
        // Outer ring (green)
        ctx.beginPath();
        ctx.arc(0, 0, this.robot.outerRadius * this.grid.scale, 0, Math.PI * 2);
        ...
        ctx.beginPath();
        ctx.moveTo(0, 0);
        ctx.lineTo(this.robot.outerRadius * this.grid.scale * 1.2, 0);
        ctx.strokeStyle = '#00ff88';
        ctx.lineWidth = 2;
        ctx.setLineDash([]);
        ctx.stroke();
        ...
        ctx.fillText(`Real (${this.realPose.x_cm.toFixed(0)},${this.realPose.y_cm.toFixed(0)})cm`, pos.x + 20, pos.y - 10);
    }

    // ── Move ───────────────────────────────────────────────────────────────────
    moveRobotTo(targetX, targetY) {
        // Gửi goal tới Jetson nếu realMode đang bật
        if (this.realMode) {
            fetch('/api/robot/goal', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ x: targetX, y: targetY })
            }).then(r => r.json()).then(d => {
                this.log(`→ Jetson goal: (${d.goal_m.x.toFixed(3)}m, ${d.goal_m.y.toFixed(3)}m)`, 'success');
            }).catch(e => this.log('Real robot: send failed', 'error'));
        }
    ...
    render() {
        ...
        // Vẽ vị trí robot thực (nếu real mode đang bật)
        if (this.realMode) {
            this.drawRealRobot();
        }
